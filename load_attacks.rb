...
csv.each do |row|
  row["source_id"] = 6 # regardless of EMR or origins patient source, putting all attacks in as origins
  ...
  @response = HTTParty.post("#{@production_root_url}#{@attacks_url}", :body => row.to_hash.to_json, :headers => { 'Content-type':'application/json', 'Authorization':'Token token="c5809928-ebe8-4b7a-ae74-40e59b61f47b"'})
